...
//id_tile foi removido. Seria necessário.
...
void Fases::Fase_Primeira::criarInimMedios(float posX, float posY) {
    ...
    if(pJog1) 
    {
       pInim->setJogador(pJog1); //é necessário?
    }*/
...
void Fases::Fase_Primeira::criarInimigos()
{
    ...
    if (!formigaSpawnPoints.empty()) {
        ...
        if (numFormigas > formigaSpawnPoints.size()) {
            numFormigas = static_cast<int>(formigaSpawnPoints.size());
        }
    ...
    if (!griloSpawnPoints.empty()) {
        ...
        if (numGrilos > griloSpawnPoints.size()) {
            numGrilos = static_cast<int>(griloSpawnPoints.size());
        }
...
void Fases::Fase_Primeira::criarObstaculos()
{
   if (!formigueiroSpawnPoints.empty()) {
        ...
        if (numObstaculos > formigueiroSpawnPoints.size()) {
            numObstaculos = static_cast<int>(formigueiroSpawnPoints.size());
        }
...
void Fases::Fase_Primeira::criarFolha(float posX, float posY) 
{
    Entidades::Obstaculos::Folha* pFolha = new Entidades::Obstaculos::Folha(sf::Vector2f(posX, posY));
